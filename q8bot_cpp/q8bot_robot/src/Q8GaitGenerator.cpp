/*
  Q8GaitGenerator.cpp - Gait generation and locomotion planning for Q8bot
  Ported from Python helpers.py by yufeng.wu0902@gmail.com
...
bool Q8GaitGenerator::generateTrajectory(GaitParams params, Direction dir, float move_p[][2], 
    ...
    if (params.y0 - params.yrange < 5) {
        Serial.println("Invalid: y below physical limit.");
        return false;
    }
    ...
    for (int i = 0; i < length; i++) {
        ...
        if (!check1 || !check2 || !check3) {
            ...
            if (params.xrange > 1 && params.yrange > 1) {
                Serial.println("IK failed, retrying with smaller step size");
